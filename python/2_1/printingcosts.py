...
def get_cost_table(ct: Dict[str, int] = None) -> Dict[str, int]:
    if ct == None:
        lines = """
    0        !   9        "   6        #  24        $  29        %  22
&  24        '   3        (  12        )  12        *  17        +  13
...
"""[1:-1].split('\n')
        ct = dict()
        for ndx, row in enumerate(lines):
            for col in range(6):
                if len(ct) < 95:
                    ct[row[col*13:col*13+1]] = int(row[col*13+3:col*13+5])
    return ct

def printingcosts(line: str) -> int:
    ct = get_cost_table()
    return sum([ct[x] for x in line])


def test_1():
    assert printingcosts('This is a short line.') == 300

def test_2():
    assert printingcosts('THIS IS A LONGER, MORE EXPENSIVE LINE.') == 707

if __name__ == '__main__':
    for line in sys.stdin:
        print(printingcosts(line.rstrip('\n')))
